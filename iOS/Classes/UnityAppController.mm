...
- (void)startUnity:(UIApplication*)application
{
    ...
    self.pebbleController = [[PebbleController alloc]init];
    self.myWatch = [self.pebbleController startController];
    
    NSLog(@"WATCH %@",self.myWatch);
    ...
    [self.myWatch appMessagesAddReceiveUpdateHandler:^BOOL(PBWatch *watch, NSDictionary *update) {
        NSNumber *movement = [update objectForKey:@(0)];
        
        int movementDirection = [movement intValue];
        ...
        [[NSUserDefaults standardUserDefaults]setObject:direction forKey:@"direction"];
